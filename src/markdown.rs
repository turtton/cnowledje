...
fn macro_open_re() -> &'static Regex {
    ...
    RE.get_or_init(|| {
        Regex::new(r#"<ac:structured-macro[^>]*\bac:name="([^"]*)"[^>]*>"#).unwrap()
    })
}
...
fn ri_page_re() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| {
        Regex::new(r#"<ri:page[^>]*\bri:content-title="([^"]*)"[^>]*/>"#).unwrap()
    })
}
...
fn ri_attachment_re() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| {
        Regex::new(r#"<ri:attachment[^>]*\bri:filename="([^"]*)"[^>]*/>"#).unwrap()
    })
}
...
/// Translate Confluence-specific XML tags into something a standard HTML
/// parser can process, or into their Markdown equivalents directly.
fn preprocess(html: &str) -> String {
    // ac:structured-macro → <div class="ac-macro" data-name="X">
    let s = macro_open_re()
        .replace_all(html, |caps: &regex::Captures| {
            format!(
                r#"<div class="ac-macro" data-macro-name="{}">"#,
                &caps[1]
            )
        })
        .into_owned();
...
/// Convert Confluence storage HTML to Markdown, capped at `max_chars`.
pub fn html_to_markdown(html: &str, max_chars: usize) -> String {
    let processed = preprocess(html);
    let document = Html::parse_fragment(&processed);

    // Parse_fragment wraps content in <html><body>; select <body>.
...
fn convert_element(elem: ElementRef<'_>, out: &mut String, ctx: &mut Ctx) {
    ...
    match tag {
        ...
        | "header" | "footer" | "nav" | "span" | "figure" => {
            // For ac-macro divs, emit a placeholder instead of recursing.
...
mod tests {
    ...
    #[test]
    fn test_heading() {
        let md = html_to_markdown("<h1>Hello</h1>", 50_000);
        assert!(md.contains("# Hello"));
    }

    #[test]
    fn test_paragraph() {
        let md = html_to_markdown("<p>Some text</p>", 50_000);
        assert!(md.contains("Some text"));
    }

    #[test]
    fn test_strong() {
        let md = html_to_markdown("<p><strong>bold</strong></p>", 50_000);
        assert!(md.contains("**bold**"));
    }

    #[test]
    fn test_link() {
        let md = html_to_markdown(r#"<a href="https://example.com">Example</a>"#, 50_000);
        assert!(md.contains("[Example](https://example.com)"));
    }

    #[test]
    fn test_unordered_list() {
        let md = html_to_markdown("<ul><li>A</li><li>B</li></ul>", 50_000);
    ...
    fn test_ordered_list() {
        let md = html_to_markdown("<ol><li>First</li><li>Second</li></ol>", 50_000);
    ...
    #[test]
    fn test_code_inline() {
        let md = html_to_markdown("<code>let x = 1;</code>", 50_000);
    ...
    fn test_macro_placeholder() {
        ...
        let md = html_to_markdown(html, 50_000);
    ...
    fn test_truncation() {
        ...
        let md = html_to_markdown(&html, 50);
        assert!(md.contains("[content truncated]"));
        // Character count of non-truncated prefix should be ≤ 50
        assert!(md.chars().count() <= 50 + "[content truncated]\n\n".len());
    }

    #[test]
    fn test_japanese_utf8() {
        let md = html_to_markdown("<p>Redisの利用方針について</p>", 50_000);
