...
について質問したときに、`cnowledje jira` で情報を集めて回答を構成するためのもの。
...
```sh
cnowledje jira --help          # 全体像
cnowledje jira search --help   # 検索の引数・オプション・例
cnowledje jira issue --help    # issue 取得の引数・オプション・例
...
1. **検索**: ユーザーの質問からキーワードを抽出し、`--project` を指定して
   `cnowledje jira search` で候補 issue を探す。必要に応じて `--status` /
   `--assignee` / `--reporter` / `--type` / `--label` で絞り込む。
2. **取得**: 候補から最も関連性の高い issue を選び `cnowledje jira issue <KEY>`
   でコメント込みの詳細を取得する。
